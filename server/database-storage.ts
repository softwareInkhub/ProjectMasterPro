import {
  ...
  Sprint, InsertSprint, SprintItem, InsertSprintItem,
  // Schema tables
...
export class DatabaseStorage implements IStorage {
  ...
  async getSprints(projectId?: string, teamId?: string, status?: string): Promise<Sprint[]> {
    try {
      ...
      if (projectId) {
        query = query.where(eq(sprints.projectId, projectId));
      }
      
      if (teamId) {
        query = query.where(eq(sprints.teamId, teamId));
      }
      
      if (status) {
        query = query.where(eq(sprints.status, status as any));
      }
      
      return await query.orderBy(sprints.startDate);
    } catch (error) {
  ...
  async deleteSprint(id: string): Promise<boolean> {
    try {
      // First, delete all sprint items associated with this sprint
      await db.delete(sprintItems).where(eq(sprintItems.sprintId, id));
  ...
  async getSprintItems(sprintId: string, itemType?: string): Promise<SprintItem[]> {
    try {
      let query = db.select().from(sprintItems).where(eq(sprintItems.sprintId, sprintId));
      
      if (itemType) {
        query = query.where(eq(sprintItems.itemType, itemType as any));
      }
      
      return await query;
    } catch (error) {
      console.error("Error getting sprint items:", error);
      return [];
    }
  }

  async getBacklogItems(projectId: string): Promise<{ epics: Epic[], stories: Story[], tasks: Task[] }> {
    try {
      // Get all epics in backlog for this project
      const backlogEpics = await db.select()
        .from(epics)
        .where(and(
          eq(epics.projectId, projectId),
          eq(epics.status, "BACKLOG")
        ));
      
      // Get all epic IDs for this project
      const projectEpicIds = (await db.select({ id: epics.id })
        .from(epics)
        .where(eq(epics.projectId, projectId)))
        .map(epic => epic.id);
      
      // Get all stories in backlog for this project's epics
      const backlogStories = await db.select()
        .from(stories)
        .where(and(
          inArray(stories.epicId, projectEpicIds),
          eq(stories.status, "BACKLOG")
        ));
      
      // Get all story IDs for this project's epics
      const epicStoryIds = (await db.select({ id: stories.id })
        .from(stories)
        .where(inArray(stories.epicId, projectEpicIds)))
        .map(story => story.id);
      
      // Get all tasks in backlog for this project's stories
      const backlogTasks = await db.select()
        .from(tasks)
        .where(and(
          inArray(tasks.storyId, epicStoryIds),
          eq(tasks.status, "TODO")  // TODO status means it's in backlog
        ));
  ...
  async addItemToSprint(item: InsertSprintItem): Promise<SprintItem> {
    try {
      const [newItem] = await db.insert(sprintItems).values(item).returning();
      
      // Update the status of the item based on its type
      if (item.itemType === "EPIC") {
        await db.update(epics)
          .set({ status: "IN_PROGRESS" })
          .where(eq(epics.id, item.itemId));
      } else if (item.itemType === "STORY") {
        await db.update(stories)
          .set({ status: "READY" })
          .where(eq(stories.id, item.itemId));
      } else if (item.itemType === "TASK") {
        await db.update(tasks)
          .set({ status: "TODO" })
          .where(eq(tasks.id, item.itemId));
      }
  ...
  async removeItemFromSprint(sprintId: string, itemType: string, itemId: string): Promise<boolean> {
    try {
      await db.delete(sprintItems)
        .where(and(
          eq(sprintItems.sprintId, sprintId),
          eq(sprintItems.itemType, itemType as any),
          eq(sprintItems.itemId, itemId)
        ));
      
      // Update the status of the item based on its type to put it back in backlog
      if (itemType === "EPIC") {
        await db.update(epics)
          .set({ status: "BACKLOG" })
          .where(eq(epics.id, itemId));
      } else if (itemType === "STORY") {
        await db.update(stories)
          .set({ status: "BACKLOG" })
          .where(eq(stories.id, itemId));
  ...
  async moveItemToSprint(itemId: string, itemType: string, fromSprintId: string, toSprintId: string): Promise<boolean> {
    try {
      // First remove the item from the current sprint
      await db.delete(sprintItems)
        .where(and(
          eq(sprintItems.sprintId, fromSprintId),
          eq(sprintItems.itemType, itemType as any),
          eq(sprintItems.itemId, itemId)
        ));
      
      // Then add it to the new sprint with the moved flag
      await db.insert(sprintItems).values({
        sprintId: toSprintId,
        itemType: itemType as any,
        itemId,
        addedFromBacklog: "NO",
        movedFromPreviousSprint: fromSprintId
      });
  ...
  async getSprintItemDetails(sprintId: string): Promise<{ 
    epics: Epic[], 
    stories: Story[], 
    tasks: Task[] 
  }> {
    try {
      ...
      const items = await this.getSprintItems(sprintId);
      ...
      // Get full details for each item type
      const epicDetails = epicIds.length > 0 
        ? await db.select().from(epics).where(inArray(epics.id, epicIds))
        : [];
      
      const storyDetails = storyIds.length > 0
        ? await db.select().from(stories).where(inArray(stories.id, storyIds))
        : [];
      ...
      console.error("Error getting sprint item details:", error);
      return { epics: [], stories: [], tasks: [] };
    }
  }

  async getActiveSprintsByUser(userId: string): Promise<Sprint[]> {
    try {
      ...
      // Get all active sprints for those teams
      return await db.select()
        .from(sprints)
        .where(and(
          inArray(sprints.teamId, teamIds),
          eq(sprints.status, "ACTIVE")
        ));
    } catch (error) {
      console.error("Error getting active sprints by user:", error);
      return [];
    }
  }

  async completeSprintAndMoveUnfinishedItems(sprintId: string, nextSprintId?: string): Promise<boolean> {
    try {
      // Get all items in this sprint
      const items = await this.getSprintItems(sprintId);
      ...
      if (nextSprintId) {
        ...
        for (const item of items) {
          ...
          if (item.itemType === "EPIC") {
            const epic = await this.getEpic(item.itemId);
            isComplete = epic?.status === "COMPLETED";
          } else if (item.itemType === "STORY") {
            const story = await this.getStory(item.itemId);
            isComplete = story?.status === "DONE";
          } else if (item.itemType === "TASK") {
            const task = await this.getTask(item.itemId);
            isComplete = task?.status === "DONE";
          }
          
          if (!isComplete) {
            // Move unfinished item to the next sprint
            await this.moveItemToSprint(item.itemId, item.itemType, sprintId, nextSprintId);
          }
        }
      } else {
